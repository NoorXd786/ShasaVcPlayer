...
from Shasa import (ASSISTANT_HANDLER, BOT_ID, BOT_USERNAME, MUSIC_BOT_NAME,
                   OWNER_ID, SUDOERS, app)
from Shasa.Database import (add_gban_user, add_off, add_on, add_sudo,
...
{ASSISTANT_HANDLER[0]}block [ Reply to a User Message] - Blocks the User from Assistant Account.
{ASSISTANT_HANDLER[0]}unblock [ Reply to a User Message] - Unblocks the User from Assistant Account.
{ASSISTANT_HANDLER[0]}approve [ Reply to a User Message] - Approves the User for DM.
{ASSISTANT_HANDLER[0]}disapprove [ Reply to a User Message] - Disapproves the User for DM.
{ASSISTANT_HANDLER[0]}pfp [ Reply to a Photo] - Changes Assistant account PFP.
{ASSISTANT_HANDLER[0]}bio [Bio text] - Changes Bio of Assistant Account.
"""
# Add Sudo Users!
...
async def useradd(_, message: Message):
    if not message.reply_to_message:
        ...
        if added:
            ...
            os.system(f"kill -9 {os.getpid()} && python3 -m Shasa")
        else:
...
async def userdel(_, message: Message):
    if not message.reply_to_message:
        ...
        if removed:
            ...
            return os.system(f"kill -9 {os.getpid()} && python3 -m Shasa")
