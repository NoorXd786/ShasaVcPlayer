...
from Shasa.Core.PyTgCalls import Queues, Shasa
from Shasa.Core.PyTgCalls.Converter import convert
from Shasa.Core.PyTgCalls.Downloader import download
from Shasa.Database import (_get_playlists, delete_playlist, get_playlist,
                            get_playlist_names, is_active_chat, save_playlist)
from Shasa.Database.queue import (add_active_chat, is_active_chat,
                                  is_music_playing, music_off, music_on,
                                  remove_active_chat)
from Shasa.Decorators.admins import AdminRightsCheckCB
from Shasa.Decorators.checker import checkerCB
from Shasa.Inline import (audio_markup, audio_markup2, download_markup,
...
from Shasa.Utilities.paste import isPreviewUp, paste_queue
...
from Shasa.Utilities.youtube import get_yt_info_id
...
async def admin_risghts(_, CallbackQuery):
    ...
    if command == "pausecb":
        ...
        await Shasa.pytgcalls.pause_stream(chat_id)
    ...
    if command == "resumecb":
        ...
        await Shasa.pytgcalls.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
    ...
    if command == "stopcb":
        ...
        await Shasa.pytgcalls.leave_group_call(chat_id)
        await CallbackQuery.message.reply_text(
    ...
    if command == "skipcb":
        ...
        if Queues.is_empty(chat_id):
            ...
            if str(finxx) != "raw":
                ...
                await Shasa.pytgcalls.change_stream(
                    chat_id,
                    InputStream(
                        InputAudioStream(
                            raw_path,
...
async def play_playlist(_, CallbackQuery):
    ...
    if not _playlist:
        ...
        for mdnoor in _playlist:
            _note = await get_playlist(user_id, mdnoor, type)
            title = _note["title"]
            ...
            if await is_active_chat(chat_id):
                ...
                raw_path = await convert(downloaded_file)
                try:
                    await Shasa.pytgcalls.join_group_call(
                        chat_id,
                        InputStream(
                            InputAudioStream(
                                raw_path,
...
async def check_playlist(_, CallbackQuery):
    ...
    if not _playlist:
        ...
        for mdnoor in _playlist:
            j += 1
            _note = await get_playlist(user_id, mdnoor, genre)
            title = _note["title"]
            duration = _note["duration"]
            msg += f"{j}- {title[:60]}\n"
...
async def del_playlist(_, CallbackQuery):
    ...
    if not _playlist:
        ...
        await CallbackQuery.answer()
        for mdnoor in _playlist:
            await delete_playlist(user_id, mdnoor, genre)
    await CallbackQuery.message.reply_text(
