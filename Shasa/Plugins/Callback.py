...
from Shasa.Core.PyTgCalls.Shasa import (join_stream, pause_stream,
...
async def play_playlist(_, CallbackQuery):
    ...
    if not _playlist:
        ...
        for_t = 0
        for_p = 0
        for lionx in _playlist:
            _note = await get_playlist(user_id, lionx, type)
            title = _note["title"]
...
async def check_playlist(_, CallbackQuery):
    ...
    if not _playlist:
        ...
        for lionx in _playlist:
            j += 1
            _note = await get_playlist(user_id, lionx, genre)
            title = _note["title"]
            duration = _note["duration"]
            msg += f"{j}- {title[:60]}\n"
...
async def del_playlist(_, CallbackQuery):
    ...
    if not _playlist:
        ...
        await CallbackQuery.answer()
        for lionx in _playlist:
            await delete_playlist(user_id, lionx, genre)
    await CallbackQuery.message.reply_text(
