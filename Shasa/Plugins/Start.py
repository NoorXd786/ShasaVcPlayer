...
from Shasa import ASSIDS, BOT_ID, MUSIC_BOT_NAME, OWNER_ID, SUDOERS, app
from Shasa import boottime as bot_start_time
from Shasa import db, random_assistant
from Shasa.Core.PyTgCalls import Shasa
from Shasa.Database import (add_nonadmin_chat, add_served_chat,
...
from Shasa.Decorators.admins import ActualAdminCB
from Shasa.Decorators.permission import PermissionCheck
from Shasa.Inline import (custommarkup, dashmarkup, setting_markup,
...
async def useradd(_, message: Message):
    ...
    await asyncio.gather(
        message.delete(),
        message.reply_text(
            f"Thanks for having me in {message.chat.title}.\n{MUSIC_BOT_NAME} is alive.\n\nFor any assistance or vchelp, checkout our support group and channel.",
            reply_markup=InlineKeyboardMarkup(out[1]),
        ),
    )
...
async def start_markup_check(_, CallbackQuery):
    ...
    if command == "LV":
        ...
        volume = 25
        try:
            await Shasa.pytgcalls.change_volume_call(c_id, volume)
    ...
    if command == "DIT":
        ...
        await CallbackQuery.answer(
            f"Shasa Disk Usage: {diske}%", show_alert=True
        )
